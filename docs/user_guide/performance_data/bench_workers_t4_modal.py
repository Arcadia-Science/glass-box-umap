"""Run the workers sweep on a Modal T4.

Sister script to ``bench_workers_h100_modal.py``; same protocol, same MNIST,
but on a cheaper T4 (16 GB) so the workers plot spans a wider GPU range.

Usage:
    modal run docs/user_guide/performance_data/bench_workers_t4_modal.py

Writes CSV to ``docs/user_guide/performance_data/workers_t4.csv``.
"""
...
app = modal.App("glass-box-umap-bench-workers-t4")
...
def run_workers_bench(epochs: int = 1, seed: int = 22) -> str:
    ...
    print(f"X.shape={X.shape} X.nbytes={X.nbytes / 1e6:.1f} MB", flush=True)
    ...
    warmup_reducer = GlassBoxUMAP(epochs=1, random_state=seed, quiet=True, num_workers=0)
...
def main(out: str = "docs/user_guide/performance_data/workers_t4.csv") -> None:
