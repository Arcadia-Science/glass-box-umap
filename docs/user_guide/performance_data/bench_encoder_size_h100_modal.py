"""Modal H100 sweep: 133K vs 5M encoder, 1 epoch, full MNIST.

Pairs with ``bench_encoder_size.py`` (the local CPU + MPS rows of the same
experiment). The combined CSVs answer "when does a GPU help?" by comparing
how wall clock scales with encoder parameter count across devices.

Usage:
    modal run docs/user_guide/performance_data/bench_encoder_size_h100_modal.py

Writes CSV to ``docs/user_guide/performance_data/encoder_size_h100.csv``.
"""
...
app = modal.App("glass-box-umap-bench-encoder-size-h100")
...
def main(out: str = "docs/user_guide/performance_data/encoder_size_h100.csv") -> None:
